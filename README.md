...
> **Status: v0.10.1.** Server, ingestion, RAG chat with citations, widget,
...
| `RERANKER_ENABLED` | Re-rank retrieved chunks with a cross-encoder before they reach the LLM, for better precision on the top results. Adds latency (runs on WASM, no GPU). Since v0.10.0 the default image leaves out the re-ranker libraries (~132 MB) to stay slim, so enabling this alone falls back to plain retrieval (with a log) — rebuild the image with `--build-arg INSTALL_RERANKER=true` to actually re-rank. The model (~24 MB) still downloads on first use, cached locally | `false` |
...
- [ ] **Phase 3 — Traction** *(in progress)*: shipped so far — 👍/👎 answer feedback · GitHub repo & PDF ingestion · secret redaction on ingest · coverage analytics in the dashboard (answer rate, top unanswered questions, most cited sections) with opt-in public demo stats · hybrid retrieval (full-text + embeddings fused by Reciprocal Rank Fusion) · optional cross-encoder re-ranking · answer streaming (SSE) · multi-turn conversations · [MCP server](https://docs.docsera.dev/#mcp-server), Discord & Slack bots so agents and chat tools can query your docs · one-command install (`npx docsera`). On the radar: Notion/Confluence connectors · cloud version (multi-tenant, usage-based billing).
