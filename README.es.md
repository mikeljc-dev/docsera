...
[![Tests](https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/mikeljc-dev/docsera/badges/tests.json)](https://github.com/mikeljc-dev/docsera/actions/workflows/ci.yml)
...
> **Estado: v0.10.1.** Server, ingesta, chat con RAG y citas, widget,
...
| Paquete | Qué hace |
|---|---|
| `packages/server` | La API: `POST /chat` (RAG con citas), `POST /chat/stream` (lo mismo, en streaming por SSE — es lo que usa el widget), `POST /ingest` (markdown/URL/sitemap/PDF/GitHub), `POST /mcp` (servidor MCP para agentes de IA), `GET /llms.txt`, bots opcionales de [Discord](https://docs.docsera.dev/#discord-bot) y [Slack](https://docs.docsera.dev/#slack-bot) (`/ask`), y sirve el widget y el dashboard como estáticos |
...
| `RERANKER_ENABLED` | Reordena los chunks recuperados con un cross-encoder antes de que lleguen al LLM, para mejor precisión en los primeros resultados. Añade latencia (corre en WASM, sin GPU). Desde v0.10.0 la imagen por defecto no incluye las librerías del re-ranker (~132 MB) para ir ligera, así que activarlo a secas cae a recuperación normal (con un aviso) — reconstruye la imagen con `--build-arg INSTALL_RERANKER=true` para reordenar de verdad. El modelo (~24 MB) sigue descargándose en el primer uso, cacheado localmente | `false` |
...
- [ ] **Fase 3 — Tracción** *(en marcha)*: entregado hasta ahora — feedback 👍/👎 en las respuestas · ingesta de repos de GitHub y de PDF · redacción de secretos en la ingesta · analíticas de cobertura en el dashboard (tasa de respuesta, top preguntas sin responder, secciones más citadas) con stats públicas opcionales para demos · búsqueda híbrida (full-text + embeddings fusionados por Reciprocal Rank Fusion) · re-ranking opcional con cross-encoder · streaming de respuestas (SSE) · conversaciones multi-turno · [servidor MCP](https://docs.docsera.dev/#mcp-server) y bots de Discord y Slack para que agentes y herramientas de chat consulten tus docs · instalación en un comando (`npx docsera`). En el radar: conectores Notion/Confluence · versión cloud (multi-tenant, billing por uso).
